...
The expected tracked `src/lib/**` count at the current handoff is 47 (it was 44
through `274d428`; `5c0de4c` added `src/lib/ai/session-readiness.ts` and its
test, and the seven-route evaluation work added
`src/lib/ai/eval-catalog-parity.test.ts`). The last command must remain empty
unless the user explicitly changes the
